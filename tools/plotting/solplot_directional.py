...
Written by George Heald, modified by Oscar Martinez and Adam Stewart
...
version 0.5 03/05/2013: Added basic support of DirectionalGain
"""
...
def getAmplPhaseCorr(p, station, ptype, corr , polar, cords, last, source):
    valsamp = None
    valsphase = None
    ...
    if polar:
        ...
        elif ptype == DDGAIN_TYPE:
            if REAL_COORD in cords and IMAG_COORD in cords:
                # for s in sources:
                valscorr = getParamDBData(p, DDPARAM_NAME_PATTERN%(ptype,corr,REAL_COORD,station, source))+1.j*getParamDBData(p, DDPARAM_NAME_PATTERN%(ptype,corr,IMAG_COORD,station, source))
...
def getAmplPhase(p, station, ptype, corrs , polar, cords, last, source):
    valsamp = {}
    valsphase = {}
    for corr in corrs:
        (valsamp[corr], valsphase[corr], averaged) = getAmplPhaseCorr(p, station, ptype, corr , polar, cords, last, source)
    ...
    if ptype==DDGAIN_TYPE:
	    times = getParamDBData(p, DDPARAM_NAME_PATTERN % (ptype, corrs[0], cords[0], station, source) , 'times')
    else:
...
op.add_option('-D','--directional',default=False,help='Look at Directional Gains.',action='store_true')
op.add_option('-S','--directionsource',default="",help='Specify which source direction gains to view.',type='string')
